...
def setup_gp(prob, regularization, mu_0=None):
    ...
    """
    from sdp_setup import get_A_inv
...
def total_cost(theta, prob, regularization, precompute=None, mu_0=None):
    from problem import generate_distances
...
def get_hess(
    ...
    if precompute is None:
        ...
        A_inv, Q_inv, R, v = precompute
    return 2 * (J.T @ Sig_inv @ J + R / prob.N)


def get_grad_hess_cost_f(
    theta, prob, return_grad=True, return_hess=False, return_cost=False
):
...
def gauss_newton(
    ...
    :returns: theta_hat, stats.
        theta_hat: estimate after convergence.
    ...
    if progressbar:
        ...
        p = progressbar.ProgressBar(max_value=max_iter)
        p.start()
    ...
    for i in range(max_iter):
        ...
        for lamda in LAMBDAS:
            ...
            if np.any(np.isnan(dtheta)):
                print("Warning: singular matrix in update calculation!")
                break
